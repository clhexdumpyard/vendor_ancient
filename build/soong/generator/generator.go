...
func init() {
	android.RegisterModuleType("ancient_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ancientExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ancientExpandVariables(ctx, dep_file)
	...
	cmd := ancientExpandVariables(ctx, String(g.properties.Cmd))
